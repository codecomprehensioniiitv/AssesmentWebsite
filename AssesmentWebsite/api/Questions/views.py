...
def questionbanklevel(request, pk=None):
    ...
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            ...
            question_bank_level_id = QuestionsBankLevel.objects.order_by('-qblid')[0].qblid
...
def getcode(request):
    ...
    if request.method == 'GET': 
        ...
        level = QuestionsBankLevel.objects.get(fqbid = question_bank_id, qlevel = queries['level'][0]).qblid
